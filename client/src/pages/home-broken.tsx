...
export default function Home() {
  ...
  // Simplified session management without infinite loops
  const isTracking = Boolean(currentSession?.isActive || activeSession?.isActive);
  
  // Only track location if we have an active session
  useEffect(() => {
    let intervalId: NodeJS.Timeout;
    
    if (isTracking && location && currentSession?.id) {
