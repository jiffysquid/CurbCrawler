...
- November 5, 2025: FIXED map camera following behavior - Camera now only follows van during active recording sessions, allowing free exploration when not recording
- November 5, 2025: COMPREHENSIVE demographics fix - Added authentic 2021 Census data for 67 Brisbane suburbs ensuring accurate population, median house prices ($595k-$1.65M), and star ratings (1-5 stars) for all future clearout schedules without fallback data
- November 23, 2025: UI POLISH - Hidden GPS debug panel from production view, reduced info panel z-index from z-[1000] to z-40 to ensure alerts/toasts appear above map overlays
- November 23, 2025: IMPLEMENTED FADE WITH AGE FEATURE - Added pathColorScheme prop to MapboxMap component with support for 'bright' (8-color cycle) and 'fade' (newer paths brighter/thicker) schemes using getPathColor utility function with data-driven Mapbox layer styling, integrated with localStorage sync from PathManagement settings
- November 23, 2025: FIXED session totals distance display - Changed formatDistance function to always show kilometers for consistency (removed meter conversion that was showing "0m" instead of "0.0km")
- November 23, 2025: IMPLEMENTED map centering on app start - Map now automatically centers on current GPS location when first acquired using dedicated hasInitialCenteredRef flag to ensure one-time centering with smooth 1.5s animation